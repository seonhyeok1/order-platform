	...
	class FailureCase {
		...
		void updateOrderStatus_ByWrongOwner_ThrowsAccessDenied() {
			...
			assertThatThrownBy(() -> orderService.updateOrderStatus(orderId, newStatus))
				...
				.isEqualTo(ErrorStatus.ORDER_ACCESS_DENIED);
		}
		...
		void updateOrderStatus_InvalidTransition_ThrowsException() {
			...
			assertThatThrownBy(() -> orderService.updateOrderStatus(orderId, invalidNewStatus))
				.isInstanceOf(GeneralException.class)
				.extracting("errorStatus")
				.isEqualTo(ErrorStatus.INVALID_ORDER_STATUS_TRANSITION);
		...
		@Test
		@DisplayName("고객이 주문 후 5분이 지난 주문을 취소하려고 하면 예외가 발생한다.")
		void updateOrderStatus_ByCustomer_After5Minutes_ThrowsException() {
			// Given
			User customer = User.builder().userId(2L).userRole(UserRole.CUSTOMER).build();

			Orders oldOrder = Orders.builder()
				.ordersId(orderId)
				.orderStatus(OrderStatus.PENDING)
				.store(store)
				.user(customer)
				.orderHistory("{}")
				.build();
			ReflectionTestUtils.setField(oldOrder, "createdAt", LocalDateTime.now().minusMinutes(10));

			when(securityUtil.getCurrentUser()).thenReturn(customer);
			when(ordersRepository.findById(orderId)).thenReturn(Optional.of(oldOrder));

			// When & Then
			assertThatThrownBy(() -> orderService.updateOrderStatus(orderId, OrderStatus.REFUNDED))
				.isInstanceOf(GeneralException.class)
				.extracting("errorStatus")
				.isEqualTo(ErrorStatus.ORDER_CANCEL_TIME_EXPIRED);
		}
	}
}